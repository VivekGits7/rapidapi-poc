...
async def _background_dump(mode: str) -> None:
    """Wrapper that runs dump_main with manage_pool=False and swallows AllKeysExhaustedError.

    The HTTP request returns immediately; this runs in the background until done.
    """
    global _running_task
    try:
        await dump_main(mode=mode, manage_pool=False)
...
async def start_dump(request: Request, body: StartDumpRequest) -> StartDumpResponse:
    ...
    _running_task = asyncio.create_task(_background_dump(body.mode))
